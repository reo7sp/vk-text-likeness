    ...
    def get_all(self):
        ...
        for user in tqdm(self.raw_users_data.members, 'ActionData.get_all: for members'):
            for post in self.raw_wall_data.posts:
                is_liked = user['id'] in post['likes']['user_ids_set']
                is_reposted = user['id'] in post['reposts']['user_ids_set']
                ...
                if is_reposted:
                    for friend in self.raw_users_data.member_friends[user['id']]:
                        ...
                        if friend_post_pair not in friend_post_pairs:
                            friend_is_liked = friend['id'] in post['likes']['user_ids_set']
                            friend_is_reposted = friend['id'] in post['reposts']['user_ids_set']
